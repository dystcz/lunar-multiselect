<div x-data="select({
    ...
    optionValue: 'value',
    optionLabel: 'label',
    value: @entangle($field['signature'].(isset($language) ? '.' . $language : null))
})" class="relative">
    <div class="relative">
        <div class="">
            <div class="relative rounded-md shadow-sm">
                <div class="absolute left-0 inset-y-0 pl-2 pr-14 w-full flex items-center overflow-hidden cursor-pointer"
                    ...
                    x-on:click="togglePopover" style="display: none;">
                    <div class="flex items-center gap-2 overflow-x-auto hide-scrollbar">
                        <span class="inline-flex text-gray-500 text-sm" x-show="selectedOptions.length"
                            x-text="model ? model.length : ''" style="display: none;"></span>
                        <template x-for="selected in selectedOptions" :key="`selected.${selected.value}`">
                            <span
                                class="inline-flex items-center py-1 pl-2 pr-1 rounded-full text-xs font-medium border border-gray-300">
                ...
                <input type="text" autocomplete="off"
                    ...
                    x-ref="select" x-on:click="togglePopover"
                    x-on:keydown.arrow-down.prevent="$event.shiftKey || getNextFocusable().focus()"
                    x-on:keydown.arrow-up.prevent="getPrevFocusable().focus()" x-bind:placeholder="getPlaceholderText()"
                    x-bind:value="getValueText()" readonly="readonly" name="model" id="20f35e630daf44dbfa4c3f68f5399d8c"
                    placeholder="Select one status">
                <div class="absolute inset-y-0 right-0 flex items-center pr-2 gap-x-2">
                    <button class="focus:outline-none"
                        x-show="!isEmptyModel() &amp;&amp; !disabled &amp;&amp; !readonly" x-on:click="clearModel"
                        type="button" style="display: none;">
                        <svg class="w-4 h-4 text-gray-500 hover:text-gray-300" xmlns="http://www.w3.org/2000/svg"
                    ...
                    <button class="focus:outline-none" x-on:click="togglePopover" type="button">
                        <svg class="w-5 h-5 text-gray-500 hover:text-gray-300" xmlns="http://www.w3.org/2000/svg"
                            fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div class="absolute w-full mt-1 rounded-lg overflow-hidden bg-white z-10 border border-gray-300" x-show="popover"
        x-on:click.outside="closePopover" x-on:keydown.escape.window="closePopover">
        <div class="px-2 my-2">
            <div class="">
                <div class="relative rounded-md ">
                    <input type="text" autocomplete="off"
                        ...
                        x-on:keydown.arrow-down.prevent="$event.shiftKey || getNextFocusable().focus()"
                        x-on:keydown.arrow-up.prevent="getPrevFocusable().focus()" placeholder="Search here"
